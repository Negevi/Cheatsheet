...
/* LEITURA DE ARQUIVO                                                         */
/*============================================================================*/
/** Abre e l� o conte�do de um arquivo wav.
 *
 * Par�metros: char* filename: nome do arquivo a ser lido.
 *             WavHeader* header: par�metro de sa�da para os dados do
 *               cabe�alho.
 *             double** data_l: par�metro de sa�da, � um ponteiro para um
 *               vetor din�mico, onde manteremos os dados lidos para o canal
 *               esquerdo. O vetor ser� alocado nesta fun��o, lembre-se de
 *               desaloc�-lo quando ele n�o for mais necess�rio! Se ocorrerem
 *               erros, o vetor N�O estar� alocado quando a fun��o retornar.
 *             double** data_r: igual ao anterior, mas para o canal direito.
 *               Se o arquivo for mono, o vetor ser� == NULL.
 *
 * Valor de Retorno: o n�mero de amostras lidas, ou 0 se ocorreu um erro. */
...
/*----------------------------------------------------------------------------*/
/** L� o chunk "RIFF" (o primeiro bloco do arquivo).
 *
 * Par�metros: FILE* file: arquivo a ser lido. Supomos que j� est� aberto.
 *             WavHeader* header: par�metro de sa�da para os dados do
 *               cabe�alho.
 *
...
/*----------------------------------------------------------------------------*/
/** L� o chunk "fmt" (o segundo bloco do arquivo).
 *
 * Par�metros: FILE* file: arquivo a ser lido. Supomos que j� est� aberto.
 *             WavHeader* header: par�metro de sa�da para os dados do
 *               cabe�alho.
...
int readFmt (FILE* file, WavHeader* header)
{
	unsigned char pre_data [8]; /* Os primeiros 8 bytes do bloco t�m informa��es sobre o resto. */
	unsigned char* data;
	int i;
	int pos = 0;
...
/*----------------------------------------------------------------------------*/
/** L� o chunk "data" (o terceiro bloco do arquivo).
 *
 * Par�metros: FILE* file: arquivo a ser lido. Supomos que j� est� aberto.
 *             WavHeader* header: par�metro de sa�da para os dados do
 *               cabe�alho.
...
{
	unsigned char pre_data [8]; /* Os primeiros 8 bytes do bloco t�m informa��es sobre o resto. */
	unsigned char samples [4]; /* 2 bytes por canal. Chamamos fread aos poucos para economizar mem�ria. */
	unsigned long i;
	int n_bytes_to_read;
	int pos = 0;
 ...
 * considerando os bytes em ordem little endian.
 *
 * Par�metros: unsigned char* buffer: l� 2 bytes daqui.
 *
...
/* ESCRITA DE ARQUIVO                                                         */
/*============================================================================*/
/** Salva as amostras em um arquivo texto, uma amostra por linha. Esta fun��o
 * � �til para visualizar os dados.
 *
 * Par�metros: char* filename: nome do arquivo a ser escrito.
 *             double* data: vetor de dados contendo as amostras.
 *             unsigned long n_samples: n�mero de amostras no vetor.
 *
 * Valor de Retorno: 1 se ocorreram erros, 0 do contr�rio. */
...
/*----------------------------------------------------------------------------*/
/** Escreve os dados de �udio em um arquivo wav. Esta fun��o N�O testa os dados
 * do cabe�alho, ent�o dados inconsistentes n�o ser�o detectados. Cuidado!
 *
 * Par�metros: char* filename: nome do arquivo a ser escrito.
 *             WavHeader* header: ponteiro para os dados do cabe�alho.
 ...
 *             WavHeader* header: dados do cabe�alho.
 *
 * Valor de Retorno: 1 se n�o ocorreram erros, 0 do contr�rio. */
...
int writeFmt (FILE* file, WavHeader* header)
{
	...
	/* O tamanho do bloco � dado por Subchunk1Size. */
...
{
	unsigned char pre_data [8]; /* Os primeiros 8 bytes do bloco t�m informa��es sobre o resto. */
	unsigned char samples [4]; /* 2 bytes por canal. Chamamos fwrite aos poucos para economizar mem�ria. */
...
/* GERA��O DE DADOS                                                           */
/*============================================================================*/
/** Gera um sinal de conte�do indeterminado.
 *
 * Par�metros: int* n_samples: n�mero de amostras a gerar para cada canal. Se
 *               for maior que o permitido, a fun��o gera um sinal menor que o
 *               pedido e modifica o valor desta vari�vel.
 *             unsigned short num_channels: n�mero de canais a gerar. Precisa
 *               ser 1 ou 2, qualquer outro valor � automaticamente tratado
 *               como 1.
 *             unsigned long sample_rate: taxa de amostragem. Se for == 0,
 *               � automaticamente ajustada para 44.1kHz.
 *             double** data_l: par�metro de sa�da, � um ponteiro para um
 *               vetor din�mico, onde manteremos os dados para o canal
 *               esquerdo. O vetor ser� alocado nesta fun��o, lembre-se de
 *               desaloc�-lo quando ele n�o for mais necess�rio!
 *             double** data_r: igual ao anterior, mas para o canal direito.
 *               Se num_channels == 1, o vetor ser� == NULL.
 *
 * Valor de Retorno: o cabe�alho para o sinal gerado (os par�metros data_l e data_r cont�m o sinal propriamente dito) */

WavHeader generateSignal (int* n_samples, unsigned short num_channels, unsigned long sample_rate, double** data_l, double** data_r)
{
	WavHeader header;
	...
	*n_samples = MIN (*n_samples, 0xffffffffUL / (2 * num_channels));
	...
	/* Preenche o cabe�alho. Faz os chunks ao contr�rio, para ter tudo consistente. */
	...
	header.subchunk1_size = 16; /* Sempre 16 para �udio PCM. */
	header.audio_format = 1; /* PCM */
	header.num_channels = num_channels;
	header.sample_rate = sample_rate;
	header.byte_rate = sample_rate * num_channels * 2;
	header.block_align = num_channels * 2;
	header.bits_per_sample = 16; /* S� trabalha com 16 bits. */
...
/*----------------------------------------------------------------------------*/
/** Preenche um vetor dado com dados aleat�rios. Cada amostra deve variar
 * entre -32767 e 32767. A fun��o N�O inicia a semente aleat�ria.
 *
 * Par�metros: double* data: vetor a preencher.
 *             int n_samples: n�mero de amostras no vetor.
 *
